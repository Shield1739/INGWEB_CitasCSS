...
{FluentPdoException, Literal, Query, Utilities
};
...
abstract class Common extends Base
{
    ...
    protected bool $isSmartJoinEnabled;

    /**
     * @param Query $fluent
     * @param       $clauses
     */
    public function __construct(Query $fluent, $clauses)
    {
     ...
     * @return $this
     */
    public function enableSmartJoin(): static
    {
        $this->isSmartJoinEnabled = true;
     ...
     * @return $this
     */
    public function disableSmartJoin(): static
    {
     ...
     *
     * @param array|string|null $condition - possibly containing ? or :name (PDO syntax)
     * @param mixed|array $parameters
     * @param string $separator - should be AND or OR
     *
     * @return $this
     */
    public function where(array|string|null $condition, mixed $parameters = [], string $separator = 'AND'): static
    {
        if ($condition === null)
        {
     ...
     * @return $this
     */
    public function whereOr(string $condition, mixed $parameters = []): static
    {
        if (is_array($condition))
        { // where(["column1 > ?" => 1, "column2 < ?" => 2])
            foreach ($condition as $key => $val)
     ...
     * @return $this
     */
    private function addJoinStatements($clause, $statement, array $parameters = []): static
    {
        if ($statement === null)
        {
            $this->joins = [];
        ...
        if (str_contains(strtoupper($statement), ' ON ') || str_contains(strtoupper($statement), ' USING'))
        {
    ...
    private function createJoinStatement($clause, $mainTable, $joinTable, string $joinAlias = ''): string
    {
        ...
        { // if the join exists don't create it again
            return '';
        }
        else
        {
        ...
        else
        {
            ...
            return " $clause $joinTable$asJoinAlias ON $joinAlias.$primaryKey = $mainTable.$mainTable" . ucfirst($foreignKey);
        }
    }
     ...
     *
     * @return array|string - the rewritten $statement (e.g. tab1.tab2:col => tab2.col)
     */
    private function createUndefinedJoins(array|string $statement): array|string
    {
    ...
    protected function buildQuery(): string
    {
        ...
        $statementsWithReferences = ['WHERE', 'SELECT', 'SELECT DISTINCT', 'GROUP BY', 'ORDER BY'];
    ...
    protected function isEscapedJoin($statement): bool
    {
        ...
        return !$this->isSmartJoinEnabled || str_contains($statement, '\.') || str_contains($statement, '\:');
    }
     ...
     * @return array
     */
    private function setJoinNameAlias($statement): array
    {
     ...
     * @return bool
     */
    private function matchTableWithJoin($table, $joinItem): bool
    {
     ...
     * @return $this
     */
    private function addRawJoins($clause, $statement, $parameters, $joinAlias, $joinTable): static
    {
        if (!$joinAlias)
        {
            $joinAlias = $joinTable;
    ...
    private function setMainTable(): string
    {
        ...
        else if (isset($this->statements['UPDATE']))
        {
     ...
     * @return mixed
     */
    private function applyTableJoin($clause, $parameters, $mainTable, $joinItem, $lastItem, $joinAlias): mixed
    {
        $alias = '';
