...
{FluentPdoException, Literal, Query, Regex, Structure, Utilities};
...
abstract class Base implements IteratorAggregate
{
    ...
    private string|bool $object;

    protected Query $fluent;

    protected PDOStatement|bool|null $result;
     ...
     * @return $this
     */
    protected function addStatement($clause, $statement, array $parameters = []): static
    {
        if ($statement === null)
        {
     ...
     * @return $this
     */
    protected function addWhereStatement($statement, string $separator = 'AND', array $parameters = []): static
    {
        if ($statement === null)
        {
     ...
     * @return $this
     */
    protected function resetClause($clause): static
    {
        $this->statements[$clause] = null;
        $this->parameters[$clause] = [];
     ...
     *
     * @return \PDOStatement|\ArrayIterator|bool|null
     *
     * @throws FluentPdoException
     */
    public function getIterator(): PDOStatement|ArrayIterator|bool|null
    {
        return $this->execute();
    }
     ...
     *
     * @return \PDOStatement|bool|int|null
     *
     * @throws FluentPdoException
     */
    public function execute(): PDOStatement|bool|int|null
    {
     ...
     * @return Structure
     */
    #[Pure] protected function getStructure(): Structure
    {
     ...
     *
     * @param string|bool $object If set to true, items are returned as stdClass, otherwise a class
     ...
     * @return $this
     */
    public function asObject(string|bool $object = true): static
    {
        $this->object = $object;
    ...
    protected function buildQuery(): string
    {
        ...
        foreach ($this->clauses as $clause => $separator)
        {
            if ($this->clauseNotEmpty($clause))
            {
                ...
                else if (is_callable($separator))
                {
                    $query .= $separator();
                }
                else
                {
                    throw new FluentPdoException("Clause '$clause' is incorrectly set to '$separator'.");
                }
            }
        }
        ...
        foreach ($this->parameters as $clauses)
        {
            ...
            if (is_array($clauses))
            {
                foreach ($clauses as $key => $value)
                {
                    ...
                    { // these are named params e.g. (':name' => 'Mark')
            ...
            else if ($clauses !== false && $clauses !== null)
            {
                $parameters[] = $clauses;
            }
        }
     ...
     * @return int|string
     */
    protected function quote($value): int|string
    {
        if (!isset($value))
        {
            return "NULL";
     ...
     * @return bool
     */
    #[Pure] private function clauseNotEmpty($clause): bool
    {
     ...
     * @return mixed
     */
    private function formatValue(mixed $val): mixed
    {
    ...
    private function prepareQuery(string $query): void
    {
        ...
        */
        ...
        if ($this->result === false)
        {
            ...
            {
                throw new FluentPdoException($this->message);
            }
        }
    }
    ...
    private function setObjectFetchMode(PDOStatement $result): void
    {
        if ($this->object !== false)
        {
            ...
            {
                $this->currentFetchMode = PDO::FETCH_CLASS | PDO::FETCH_PROPS_LATE;
        ...
        else if ($this->fluent->getPdo()->getAttribute(PDO::ATTR_DEFAULT_FETCH_MODE) === PDO::FETCH_BOTH)
        {
